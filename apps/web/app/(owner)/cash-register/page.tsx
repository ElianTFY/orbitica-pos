      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#8E929E]">Apertura de turno, arqueo en tiempo real y cierre con reporte de descuadre</p>
          </div>
          <div>
        ...
        {session && session.status === "OPEN" ? (
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <Card className="border-l-4 border-l-[#0EA5FF]">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[#8E929E] font-medium uppercase">Fondo Inicial</span>
                  <Banknote className="w-4 h-4 text-[#0EA5FF]" />
                </div>
                <div className="mt-3">
                  <span className="text-2xl font-bold text-white">{formatCRC(session.initial_cash_amount)}</span>
                  <span className="text-[11px] text-[#8E929E] block">Apertura: {session.opened_at}</span>
                </div>
              </Card>

              <Card className="border-l-4 border-l-emerald-500">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[#8E929E] font-medium uppercase">Ventas en Efectivo</span>
                  <DollarSign className="w-4 h-4 text-emerald-400" />
                </div>
                <div className="mt-3">
                  <span className="text-2xl font-bold text-emerald-400">{formatCRC(session.sales_cash)}</span>
                  <span className="text-[11px] text-[#8E929E] block">Ingresado al cajón</span>
                </div>
              </Card>

              <Card className="border-l-4 border-l-purple-500">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[#8E929E] font-medium uppercase">SINPE Móvil</span>
                  <Smartphone className="w-4 h-4 text-purple-400" />
                </div>
                <div className="mt-3">
                  <span className="text-2xl font-bold text-purple-400">{formatCRC(session.sales_sinpe)}</span>
                  <span className="text-[11px] text-[#8E929E] block">Bancarizado / Electrónico</span>
                </div>
              </Card>

              <Card className="border-l-4 border-l-cyan-500">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-[#8E929E] font-medium uppercase">Efectivo Esperado</span>
                  <CheckCircle className="w-4 h-4 text-cyan-400" />
                </div>
                <div className="mt-3">
                  <span className="text-2xl font-bold text-cyan-400">{formatCRC(session.expected_cash_amount)}</span>
                  <span className="text-[11px] text-[#8E929E] block">Fondo + Ventas Efectivo</span>
                </div>
              </Card>
            </div>
            ...
            <Card className="p-6">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex justify-between py-2 border-b border-[#26282E]">
                  <span className="text-[#8E929E]">Responsable del Turno:</span>
                  <span className="text-white font-semibold">{session.user_name}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#26282E]">
                  <span className="text-[#8E929E]">Caja / Terminal:</span>
                  <span className="text-white font-mono">Caja Principal 01 (POS-00001)</span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#26282E]">
                  <span className="text-[#8E929E]">Total de Ventas Tarjeta / Datáfono:</span>
                  <span className="text-white font-mono">{formatCRC(session.sales_card)}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#26282E] text-sm font-bold">
                  <span className="text-white">Total Facturado en Turno:</span>
                  <span className="text-[#0EA5FF] font-mono">
          ...
          <Card className="py-12 flex flex-col items-center justify-center text-center space-y-4">
            <div className="p-4 bg-[#1A1B1F] rounded-full border border-[#26282E] text-[#8E929E]">
              <Lock className="w-8 h-8" />
            </div>
            <div className="space-y-1 max-w-sm">
              <h3 className="text-base font-bold text-white">No hay turno de caja activo</h3>
              <p className="text-xs text-[#8E929E]">Debes registrar la apertura de caja y el fondo inicial para operar el POS.</p>
            </div>
            <Button variant="primary" onClick={() => setIsOpenModalOpen(true)}>
              <Unlock className="w-4 h-4 mr-2" />
        ...
        <form onSubmit={handleCloseSession} className="space-y-4">
          <div className="p-4 bg-[#1A1B1F] border border-[#26282E] rounded-xl space-y-2 text-xs">
            <div className="flex justify-between">
              <span className="text-[#8E929E]">Efectivo Teórico Esperado:</span>
              <span className="font-bold text-white font-mono">{formatCRC(expectedNum)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#8E929E]">Ventas SINPE Móvil:</span>
            ...
            <div
              className={`p-3 rounded-xl border flex items-center justify-between text-xs font-semibold ${
                difference === 0
                  ...
                  : difference > 0
                  ? "bg-[#0EA5FF]/10 border-[#0EA5FF]/30 text-[#0EA5FF]"
