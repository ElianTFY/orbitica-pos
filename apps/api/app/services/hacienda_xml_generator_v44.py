...
class HaciendaXMLGeneratorV44:
    ...
    def generate_xml(
        ...
        etree.SubElement(root, f"{{{ns}}}CodigoActividad").text = str(org.economic_activity_code).zfill(6)[:6]
        ...
        etree.SubElement(ubicacion, f"{{{ns}}}Provincia").text = str(org.province_code)[:1]
        etree.SubElement(ubicacion, f"{{{ns}}}Canton").text = str(org.canton_code).zfill(2)[:2]
        etree.SubElement(ubicacion, f"{{{ns}}}Distrito").text = str(org.district_code).zfill(2)[:2]
        ...
        for item in sale.items:
            ...
            # Codigo CAByS (13 digits)
            etree.SubElement(line_elem, f"{{{ns}}}CodigoCabys").text = "0000000000000"

            # Codigo Comercial
            if item.product_sku:
                cod_com = etree.SubElement(line_elem, f"{{{ns}}}CodigoComercial")
                etree.SubElement(cod_com, f"{{{ns}}}Tipo").text = "04"  # 04=Uso Interno
                etree.SubElement(cod_com, f"{{{ns}}}Codigo").text = str(item.product_sku)[:20]

            etree.SubElement(line_elem, f"{{{ns}}}Cantidad").text = fmt_money(item.quantity, 3)
            etree.SubElement(line_elem, f"{{{ns}}}UnidadMedida").text = "Unid"
            ...
            # Descuento
            if item.discount_amount and item.discount_amount > Decimal("0.00"):
                desc_elem = etree.SubElement(line_elem, f"{{{ns}}}Descuento")
                etree.SubElement(desc_elem, f"{{{ns}}}MontoDescuento").text = fmt_money_2(item.discount_amount)
                etree.SubElement(desc_elem, f"{{{ns}}}NaturalezaDescuento").text = "Descuento comercial POS"
                tot_desc += item.discount_amount

            # Subtotal linea
            subtotal_line = monto_total_linea_bruta - item.discount_amount
            etree.SubElement(line_elem, f"{{{ns}}}SubTotal").text = fmt_money_2(subtotal_line)

            # Base imponible sin IVA
            tax_rate_pct = item.tax_rate or Decimal("13.00")
            if tax_rate_pct > Decimal("0.00"):
                ...
                etree.SubElement(imp_elem, f"{{{ns}}}Codigo").text = "01"  # 01=IVA
                etree.SubElement(imp_elem, f"{{{ns}}}CodigoTarifa").text = "01"  # 01=13%
        ...
        if reference_info:
            ...
            etree.SubElement(ref_elem, f"{{{ns}}}Codigo").text = reference_info.get("code", "01")  # 01=Anula documento de referencia
